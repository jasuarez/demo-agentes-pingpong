...
/**
 * Agente que contesta con un pong al agente que le envia un ping. Antes de
 * contestarle carga al sistema durante un segundo.
 * 
 * @author jota
 * @version $Revision$
 */
public class PongAgentLoad extends RouterClientAction {
	/**
	 ...
	 */
	public PongAgentLoad(int id, Address myAddress, Address rterAddress,
			Address regAddres) {
		super();
	...
	/**
	 * @see Abstract.AgentAction#processMessage(java.lang.String,
	 *      java.lang.Object)
	 */
	...
	public boolean Act(Object obj) {
		try {
			...
			int contenido = Integer.parseInt(kqml.getValue("content"));
			String receiver = kqml.getValue("sender");
			if (perf.equals("achieve")) {
				String sendMsg = "(reply :sender " + getName() + " :receiver "
						+ receiver + " :language java :ontology none :content "
						+ String.valueOf(_fl.load(contenido)) + ")";
				sendMessage(sendMsg);
				yield();
			}
		} catch (Exception e) {
