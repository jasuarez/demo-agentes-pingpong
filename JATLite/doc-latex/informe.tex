...
presenta la filosofía de funcionamiento de dicha plataforma, y cómo se ha medido
...
\textbf{JATLite} en función del número de agentes que contiene. La idea consiste
...
\section{Descripción de la plataforma \textbf{JATLite}}
\textbf{JATLite}\cite{jatlite} es una herramienta basada en Java para la
comunicación de agentes empleando el lenguaje de comunicación KQML\cite{kqml}.

\textbf{JATLite} está formada por varias capas que permiten decidir en qué nivel
queremos empezar a desarrollar los agentes (véase figura \ref{fig:jatlite}). En
...
configuración, permite definir en qué puerto se puede registrar un agente y
cual es el puerto para comunicarse con el enrutador. De esta forma, cada vez
que creemos un agente tendremos que especificarle la dirección del enrutador y
la dirección del registrador (además de la dirección del propio agente que
creamos).
...
\emph{RouterClientAction}. Entre otros métodos, en esta subclase nueva debemos
redefinir un método (\emph{Act})que será el invocado por el enrutador cada vez
que llegue un nuevo mensaje.

\subsection{Comunicación de los agentes}
Como hemos mencionado anteriormente, en \textbf{JATLite} la comunicación se
realiza a través de los enrutadores. Así, primero creamos los enrutadores (en
nuestro proyecto sólo hemos tenemos uno al que se conectan todos los agentes) y
posteriormente creamos los distintos agentes que se conectarán al enrutador.
Una de las facilidades que provee el enrutador es el almacenamiento persistente
de los mensajes cuando el agente destinatario no está disponible. Así, si un
cuando un agente envía un mensaje a otro agente que en ese momento no se
encuentra disponible, el enrutador almacenará su mensaje para entregarlo cuando
...
agentes hayan finalizado sus medidas. Hemos empleado el patrón de diseño
...
funcionamiento en cada estado es el siguiente: al iniciarse el estado se envía
un mensaje ping al agente correspondiente. El enrutador de la plataforma
\textbf{JATLite}, al recibir la respuesta, invoca al método \emph{Act} del
agente, el cual delega en el método \emph{Act} del estado. Este método comprueba
si es necesario cambiar al siguiente estado, en cuyo caso cambiaría; en caso
contrario volvería a lanzar otro ping, y así sucesivamente. Así, el primer
estado sería PingStartingState, luego pasaría a PingMeasuringState y finalmente
...
El programa está formado por tres paquetes: \emph{pingpong.jatlite.ping}, que
...
útiles (ver figura \ref{fig:paquetes}).
